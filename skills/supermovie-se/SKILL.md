...
`public/se/` の中身を確認。空、または必要ファイルが不足している場合は、次の優先順位で素材を準備する。

1. `SUPERMOVIE_SE_SOURCE_DIR` env が設定されており参照可能なら、そのディレクトリから `<PROJECT>/public/se/` にコピーする。
2. `SUPERMOVIE_SE_SOURCE_DIR` が未設定、または参照先が存在しない場合、必要ファイル（§2-2 SE素材カタログ参照）を `<PROJECT>/public/se/` に**手動配置**する。
3. 素材配布元の正式決定は **[Roku判断待ち]** として扱い、machine-local な絶対 path を doc に書かない。

```bash
mkdir -p "<PROJECT>/public/se"

# P0 guard: SUPERMOVIE_SE_SOURCE_DIR が未設定または参照不可なら copy しない。
# guard なしで `cp "$SUPERMOVIE_SE_SOURCE_DIR"/*` を走らせると、env 未設定時に bash が `/*`
# (root の /*) を展開して system 全体から copy が走る destructive 穴があるため。
if [ -n "${SUPERMOVIE_SE_SOURCE_DIR:-}" ] && [ -d "$SUPERMOVIE_SE_SOURCE_DIR" ]; then
    cp "$SUPERMOVIE_SE_SOURCE_DIR"/* "<PROJECT>/public/se/"
else
    echo "[skip] SUPERMOVIE_SE_SOURCE_DIR 未設定または参照不可 → 手動配置 + missing report" >&2
fi
```

`SUPERMOVIE_SE_SOURCE_DIR` が未設定、参照不可、または必要ファイルが不足している場合は、コピーを継続せず **missing report** に以下を記載する:
- `missing_files`: §2-2 カタログのうち `public/se/` に不在のファイル list
- `SUPERMOVIE_SE_SOURCE_DIR` の状態 (未設定 / 設定済みだが参照不可 / 参照可だがファイル不足)
- 手動配置が必要な旨と配布元の `[Roku判断待ち]` ステータス
...
| ファイル存在 | public/se/ に全ファイルあり | missing report に記載: `missing_files` (不在ファイル list) / `SUPERMOVIE_SE_SOURCE_DIR` の状態 / 手動配置必要の旨。machine-local 絶対 path は記載しない |
...
| SE素材が不足 | §Phase 2-1 の素材チェック手順に従う: `SUPERMOVIE_SE_SOURCE_DIR` env 経由のコピー → 未設定なら手動配置 → missing report 出力。配布元の正式決定は **[Roku判断待ち]** |
