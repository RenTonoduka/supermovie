...
- user_content (transcript / segments / chunk text / telop raw):
  - human stdout: **debug opt-in flag (`--unsafe-show-user-content` 等) 時のみ raw 出力可、default では length / first-N-chars / hash のみ表示** (現 `voicevox_narration.py:626` chunk text human log は migration 対象)。
  - external structured log / json tail: `length` / `hash` のみ、raw 禁止。
...
|---|---|---|---|
| `success` | `ok` | (空 or domain-specific) | [generate_slide_plan.py:191](../template/scripts/generate_slide_plan.py) |
| `api_key_skipped` | `skipped` | `api_key_missing` | [generate_slide_plan.py](../template/scripts/generate_slide_plan.py) |
| `engine_skipped` | `skipped` | `engine_unavailable` | [voicevox_narration.py:788](../template/scripts/voicevox_narration.py) |
| `dry_run` | `dry_run` | (現状通り) | [generate_slide_plan.py:279](../template/scripts/generate_slide_plan.py) |
| `error` / variants | `error` | error 種別を `category` に格納 | 各 error path |
